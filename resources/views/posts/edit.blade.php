           ...
           <aside class="relative flex h-screen w-full max-w-xl flex-col overflow-y-auto bg-white shadow-2xl">
            <header class="sticky top-0 z-20 border-b bg-white/90 backdrop-blur">
                <div class="flex items-center gap-3 px-6 py-4">
                    <a href="{{ url()->previous() }}" class="rounded-full p-2 transition hover:bg-gray-100">
                        <x-lucide-arrow-left class="h-5 w-5" />
                    </a>
                    <div>
                        <h2 class="text-lg font-bold text-gray-900">Editar post</h2>
                        <p class="text-sm text-gray-500">Mantenha o conteúdo dentro do limite de 1200 caracteres.</p>
                    </div>
                </div>
            </header>
            ...
            <div class="flex-1 p-6">
                <form method="POST" action="{{ route('posts.update', $post) }}">
                    @method('PUT')
                    @include('posts._form', ['post' => $post, 'buttonText' => 'Salvar alterações', 'maxBody' => 1200])
                </form>
            </div>
        </aside>
    </div>
